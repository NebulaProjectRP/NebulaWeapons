...
SWEP.Category				= "ASAPGaming" --The category.  Please, just choose something generic or something I've already done if you plan on only doing like one swep.
...
SWEP.PrintName				= "Dart Pistol - Confetti"		-- Weapon name (Shown on HUD)	
SWEP.Skin 					= 3
...
--Firing related
SWEP.Primary.Sound 			= Sound("garrysmod/balloon_pop_cute.wav")				-- This is the sound of the weapon, when you shoot.
...
SWEP.Primary.RPM_Burst				= 60					-- RPM for semi-automatic or burst fire.  This is in Rounds Per Minute / RPM
...
SWEP.ProjectileEntity = "confetti_exp" --Entity to shoot
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_dartpistol_asap.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_dartpistol_asap.mdl" -- Worldmodel path
...
SWEP.MuzzleFlashEffect = "balloon_pop" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.

--Tracer Stuff

SWEP.Tracer				= 1		--Bullet tracer.  TracerName overrides this.
SWEP.TracerName 		= "balloon_pop" 	--Change to a string of your tracer name.  Can be custom.
...
function SWEP:Initialize()
	baseclass.Get("tfa_gun_base").Initialize(self)
	hook.Add("EntityTakeDamage", self, function(_, ent, dmg)
		if not (ent:IsNPC() or ent:IsPlayer()) then return end
		local att = dmg:GetAttacker()
		if (att == self:GetOwner() and IsValid(att:GetActiveWeapon()) and att:GetActiveWeapon() == self) then
			ent:CreateConfetti(3)

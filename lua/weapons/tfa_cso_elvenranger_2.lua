...
SWEP.PrintName = "AWP Elven Ranger - Midnight" -- Weapon name (Shown on HUD)
...
--[[VIEWMODEL]]
--
SWEP.ViewModel = "models/weapons/tfa_cso/c_elven_ranger_asap.mdl" --Viewmodel path
...
--
SWEP.Skin = 1
SWEP.WorldModel = "models/weapons/tfa_cso/w_elven_ranger_asap.mdl" -- Weapon world model path
...
SWEP.TracerName = "cso_tra_elv_rng_2" --Change to a string of your tracer name.  Can be custom.
...
function SWEP:Reload(rel)
    baseclass.Get("tfa_gun_base").Reload(self, rel)

    if (rel and IsValid(self._target)) then
        if (self._target:InArena()) then
            self._target = nil
        ...
        ply:ScreenFade(SCREENFADE.OUT, color_white, .15, 0)
        ply:SetFOV(30, 0)
        ply:SetFOV(0, .15)
        ply:SetPos(self._target:GetPos())
		if SERVER then
			net.Start("ElvenRanger.CreateNotification")
			net.WriteBool(false)
			net.Send(self._target)
		end
        self._target = nil
    end
end
...
if SERVER then
    util.AddNetworkString("ElvenRanger.CreateNotification")
end

function SWEP:PrimaryAttack()
    local empty = self:Clip1() == 0
    baseclass.Get("tfa_gun_base").PrimaryAttack(self)
    local tr = self:GetOwner():GetEyeTrace().Entity
    if (empty) then return end

    if (tr:IsPlayer()) then
        self._target = tr
        self._target:EmitSound("buttons/blip2.wav")

        if SERVER then
            net.Start("ElvenRanger.CreateNotification")
            net.WriteBool(true)
            net.Send(tr)
            DarkRP.notify(self._target, 1, 5, "You feel something itchy in your neck that bleeps")
            local hookName = tr:SteamID64() .. "_deathProof"

            hook.Add("PlayerDeath", hookName, function(ply, dmg)
                if not IsValid(tr) or ply == tr then
...
net.Receive("ElvenRanger.CreateNotification", function()
    local b = net.ReadBool()

    if not b then
        hook.Remove("HUDPaint", "ASPA.Rankeg")

        return
    end

    hook.Add("HUDPaint", "ASPA.Rankeg", function()
        ...
        local _, ty = draw.SimpleText("You were tagged", "Gangs.Medium", ScrW() / 2, 96, color_white, 1, 1)
        draw.SimpleText("Someone might teleport to you", "Gangs.Small", ScrW() / 2, 96 + ty, color_white, 1, 1)
    end)
end)
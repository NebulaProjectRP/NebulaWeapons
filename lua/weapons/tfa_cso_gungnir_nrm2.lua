...
SWEP.PrintName = "Ripper Gungnir" -- Weapon name (Shown on HUD)
...
--[[VIEWMODEL]]
--
SWEP.ViewModel = "models/weapons/tfa_cso/c_gungnir_nrm_asap.mdl" --Viewmodel path
...
--[[WORLDMODEL]]
--
SWEP.WorldModel = "models/weapons/tfa_cso/w_gungnira_asap.mdl" -- Worldmodel path
...
SWEP.TracerName = "tra_gungnir_nrm_pink" --Change to a string of your tracer name.  Can be custom.
...
function SWEP:SecondaryAttack(...)
    ...
    if IsFirstTimePredicted() then
		if (self:Clip1() < 5) then
			if CLIENT then
				surface.PlaySound("csgo/exosuit_long_jump.wav")
				self._holdingRight = nil
			end
			return
		end
		--self:EmitSound(string soundName, number soundLevel=75, number pitchPercent=100, number volume=1, number channel=CHAN_AUTO)
		self:EmitSound("weapons/raygun/wpn_ray_reload_close.wav", 75, 80)
		self:GetOwner():SetFOV(50, 2)
        self._holdingRight = CurTime() + 2
    end
...
function SWEP:PrimaryAttack(...)
	if (self._holdingRight) then return end
	BaseClass.PrimaryAttack(self, ...)
    self:Wait(.31, function()
...
function SWEP:Think()
    ...
    if (not self._isReady and self._holdingRight and self:GetOwner():KeyDown(IN_ATTACK2) and self._holdingRight < CurTime()) then
        self._isReady = true
    elseif (self._isReady and not self:GetOwner():KeyDown(IN_ATTACK2)) then
        self._holdingRight = nil
        self._isReady = nil
		self:GetOwner():SetFOV(0, .1)
        local tr = util.QuickTrace(self:GetOwner():GetShootPos(), self:GetOwner():GetAimVector() * 2048, self:GetOwner())

        if (SERVER and (tr.Entity:IsPlayer() or tr.Entity:IsNPC())) then
            local dmg = DamageInfo()
            dmg:SetDamage(self:Clip1() * 20)
            dmg:SetAttacker(self:GetOwner())
            dmg:SetInflictor(self)
            dmg:SetDamageType(DMG_DISSOLVE)
            tr.Entity:TakeDamageInfo(dmg)
        end
		self:EmitSound("weapons/raygun/proj_hit.wav")

        local eff = EffectData()
        eff:SetStart(self:GetOwner():GetShootPos())
        eff:SetEntity(self)
		eff:SetOrigin(tr.HitPos)
        eff:SetAttachment(1)
        eff:SetMagnitude(5)
        util.Effect("tra_gungnir_nrm_pink", eff, true, true)

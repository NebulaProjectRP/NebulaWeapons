...
SWEP.PrintName = "Shadow Hammer"
SWEP.Author = "Kamikaze" --Author Tooltip
SWEP.ViewModel = "models/weapons/tfa_cso/c_stormgiant_v8_asap.mdl"
SWEP.WorldModel = "models/weapons/tfa_cso/w_stormgiant_v8_asap.mdl"
...
function SWEP:SecondaryAttack()
	...
	if IsFirstTimePredicted() then
		timer.Simple(self.Secondary.Attacks[1].delay, function()
			local eff = EffectData()
			local tr = util.QuickTrace(self:GetOwner():GetShootPos(), self:GetOwner():GetAimVector() * 160, self:GetOwner())
			eff:SetOrigin(tr.HitPos)
			util.Effect("eff_shuriken_elect", eff, true, true)
			if SERVER then
				...
				dmg:SetDamage(300)
				for k, v in pairs(ents.FindInSphere(tr.HitPos, 96)) do

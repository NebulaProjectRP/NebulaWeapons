...
function ENT:Initialize()
    ...
    self:SetEnabled(not self:HasSpawnFlags(2))

    -- safe bet?
    if WireLib and WireLib.CreateInputs then
        self.Outputs = Wire_CreateOutputs(self, {"Out"})

        self.Inputs = WireLib.CreateSpecialInputs(self, {"Red", "Green", "Blue", "RGB", "Height_Add", "NoFallDmg", "Sound", "On", "Target_Pos", "Target_Ent", "Target_Name"}, {"NORMAL", "NORMAL", "NORMAL", "VECTOR", "NORMAL", "NORMAL", "STRING", "NORMAL", "VECTOR", "ENTITY", "STRING"})
    end
end
...
function ENT:SetupDataTables()
    ...
    --, 		Edit = { type = "EntitySelect", order = 5 } }  );
    ...
    --, 	Edit = { type = "Generic", 		order = 4 } }  );
    self:NetworkVar("String", 3, "SoundName", {
        KeyName = "soundname"
    })
...
function ENT:OnTargetChanged(var, old, new)
    ...
    if _type == "Player" or _type == "Vehicle" or _type == "Weapon" or _type == "NPC" then
        _type = "Entity"
    end
...
-- target, starting point, time to get there
...
-- target, starting point, time to get there, fraction of jump
...
function ENT:StartTouch(entity)
    ...
    if entity:IsValid() then
        ...
        if not entity:IsPlayer() and not entity:IsNPC() and entphys:IsValid() then
...
function ENT:KeyValue(key, value)
    ...
    -- NetworkVar keyvalues now working?
